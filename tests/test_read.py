...
def compare_zslice_coord(zgy_filename, tolerance):
    with segyio.open(SGY_FILE) as segyfile:
        reader = SeismicReader(zgy_filename)
...
def test_read_zslice():
    ...
    compare_zslice_coord(ZGY_FILE, tolerance=1e-5)


def compare_subvolume(zgy_filename, tolerance):
    min_il, max_il = 2,  3
    min_xl, max_xl = 1,  2
    min_z,  max_z = 10, 20
...
def test_read_subvolume():
    compare_subvolume(ZGY_FILE, tolerance=1e-5)


def compare_volume(zgy_filename, tolerance):
    vol_zgy = SeismicReader(zgy_filename).read_volume()
    vol_segy = segyio.tools.cube(SGY_FILE)
    assert np.allclose(vol_zgy, vol_segy, rtol=tolerance)

def test_read_volume():
    compare_volume(ZGY_FILE, tolerance=1e-5)
